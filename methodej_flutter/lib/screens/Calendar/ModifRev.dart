...
class ModifRev extends StatefulWidget {
  ModifRev({this.item});
  final item;

  @override
  State<ModifRev> createState() => _ModifRevState(item: this.item);
}

class _ModifRevState extends State<ModifRev> {
  _ModifRevState({required this.item});
        ...
        if (jour >= premierJour) {
          if (DateTime(year, month, jourCalcule).compareTo(item
            ...
            return unElementCalendrierTypo(jourCalcule.toString(), 'jourPasse',
                Colors.transparent, pastille, selected); //jourPasse
          }
                      ...
                      onPressed: () async {
                        ...
                        bool result = await updateRevision(
                            revisionInfo: new Revision(
                                id: item
                                    .revisions?[item.numRevActuel as int].id,
                        ...
                        for (var i = 0;
                            i < (item.revisions as List<Revision>).length;
                            i++) {
                          if (i == item.numRevActuel) {
                            Revision newRev = Revision(
                                id: (item.revisions as List<Revision>)[i].id,
                                datePrevu: dateChoisi,
                                dateFait: DateTime(0000));
                            listeRev.add(newRev);
                          } else {
                            listeRev.add((item.revisions as List<Revision>)[i]);
                          }
                        }
                        ...
                        newCourse.numRevActuel = item.numRevActuel;
